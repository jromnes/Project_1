...
class MCSimulation:
    ...
    def calc_cumulative_return(self):
       ...
       # Calculate the mean and standard deviation of daily returns for each stock
        ...
        mean_returns = np.nanmean(daily_returns, axis=0).tolist()
        std_returns = np.nanstd(daily_returns, axis=0).tolist()
        ...
        for n in range(self.nSim):
            ...
            sim_df = pd.DataFrame(simvals).T.ffill().pct_change()

            # Use the dot function with the weights to multiply weights with each column's simulated daily returns
            sim_df = sim_df.dot(self.weights)
            # Calculate the normalized, cumulative return series
            portfolio_cumulative_returns = pd.concat([portfolio_cumulative_returns, (1 + sim_df.fillna(0)).cumprod()], axis=1)
